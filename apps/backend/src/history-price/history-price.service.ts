...
export class HistoryPriceService {
  ...
  async query(
    ...
    const raw = await queryHistoryPrice(productUrl)
    ...
    // 保存到数据库（upsert：同一用户同一 URL 只保留一条）
    try {
      ...
      await this.prisma.historyPriceQuery.upsert({
        where: {
          user_id_product_url: { user_id: userId, product_url: productUrl },
        },
        create: {
          user_id: userId,
  ...
  async refresh(
    ...
    const raw = await queryHistoryPrice(record.product_url)
    if (!raw) {
      throw new Error('查询历史价格失败')
    }
